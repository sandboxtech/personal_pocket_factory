...
-- ══ half_length ══
-- 长度 = 每级步长 × (等级和 + 加成) × 2，配置是 (32, 8, 4)，即【长 = 16 × (等级和 + 4)】。
...
check('等级 0 → 半长 32（长 64）',   geo.half_length(0, 32, 8, 4), 32)
check('等级 1 → 半长 40（长 80）',   geo.half_length(1, 32, 8, 4), 40)
check('等级 12（集齐各 1 点）→ 半长 128（长 256）', geo.half_length(12, 32, 8, 4), 128)
check('等级 24（各 10 点）→ 半长 224（长 448）',    geo.half_length(24, 32, 8, 4), 224)
check('等级 84（各 100 万）→ 半长 704（长 1408）',  geo.half_length(84, 32, 8, 4), 704)

-- 逐级递增：绝不能出现"升了一级长度没变"的区间
for lv = 0, 40 do
    check(('等级 %d → %d 每级都真的变长'):format(lv, lv + 1),
        geo.half_length(lv + 1, 32, 8, 4) - geo.half_length(lv, 32, 8, 4), 8)
end

-- 下限只是脏数据的兜底：等级为负时不能返回 0 或负数，
-- 否则 tile_at 会把整条环判成 void，玩家掉进一个一格地板都没有的世界。
check('等级 -5(脏数据) → 夹到下限 32', geo.half_length(-5, 32, 8, 4), 32)
check('等级 -100(脏数据) → 夹到下限 32', geo.half_length(-100, 32, 8, 4), 32)

-- 公式对照：长度必须严格等于 (等级和 + 4) × 16
for lv = 0, 84 do
    check(('公式对照 等级 %d'):format(lv), 2 * geo.half_length(lv, 32, 8, 4), (lv + 4) * 16)
end
